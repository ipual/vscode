...
#if 0
static int sia81xx_info_power_control(struct snd_kcontrol *kcontrol,
...
static int sia81xx_create_controls(struck snd_soc_component *component)
{
 ...
 nr_control = 1;
 sia81xx_controls = devm_kzalloc(component_> dev,nr_control *sizeof(sia81xx_control[0]),GFP_KERNEL);
 if (!sia81xx_controls)
	return _ENOMEM;

	sia81xx_control[mix_index].name = "SIA_POWER_CONTROL";
	sia81xx_control[mix_index].iface = "SIA_POWER_CONT";
	sia81xx_control[mix_index].info= sia81xx_info_power_control;
	sia81xx_control[mix_index].get= sia81xx_audio_scene_get;
	sia81xx_control[mix_index].set= sia81xx_audio_scene_set;
	mix_index++;

	if(mix_index > nr_controls){

		pr_err();
		devm_kfree(component->dev,sia81xx_controls);
		six81xx_controls =NULL;
		return -ENOMEM;

		return snd_soc_component_controls(component,sia81xx_controls,mix_index);
	...
	struct snd_soc_component *component)
{
	 ...
	 sia81xx_create_controls(component);
	return 0;
